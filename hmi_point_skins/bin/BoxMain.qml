...
Item {
    ...
    Timer{
        ...
        interval: 200
        onTriggered: {
//            listView.contentY = 30 * gIndex1 + listView.spacing * (gIndex1)

            console.log(gIndex1, listView.contentY)
        }
    }

    function updateData(filter, isSearch){
        var datas = Hmibox.getQmlDatas()
//        if(listModels.count !== 0 && datas.length !== listModels.count)
//        {
//            console.log("error: datas.length !== listModels.count", datas.length , listModels.count)
//            return
//        }
...
//                            datas[i].signales[j].value)
                ...
                if(filter != "") if(map2["name"].toLowerCase().indexOf(filter.toLowerCase()) != -1) findMsg = true
            ...
            if(filter != "")
            {
                if(!findIdName && !findMsg) continue // 都没有找到 过滤
            }
