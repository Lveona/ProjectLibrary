...
void HMIBox::init(QQuickView* v)
{
    ...
    _search = "";
    // read dbc
    QString datas = readFile("../../hmi_gauge_skins/bin/DBCData");
...
void HMIBox::handleSearch()
{
    m_qml_datas_search.clear();
    ...
    if(_search.isEmpty())
    {
        ...
        {
            m_qml_datas_search.append(a); // add
        }
    }
    else{
        int ai = 0;
        ...
        foreach(QVariant a, m_qml_datas)
        {
            ...
            bool find = false;
            if(ma.value("idName").toString().toLower().contains(_search.toLower()))
            {
                find = true;  // find id
            }
            else{
                ...
                foreach (QVariant b, signales) {
                    ...
                    if(mb.value("name").toString().toLower().contains(_search.toLower()))
                    {
                        find = true;
                        break;
                    }
            ...
            if(find) m_qml_datas_search.append(m_qml_datas[ai]); // add
            ai++;
        }
    }
    emit updateDatas();
...
{
    writeFile(QString("0:%1").arg(v), "../../hmi_gauge_skins/bin/dataPM.txt");
}

void HMIBox::setSearch(QString flag)
{
    _search = flag;
